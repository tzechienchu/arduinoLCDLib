...
void TFTLCD::setTextSize(uint8_t s) {          //◊÷ÃÂ¥Û–°
  textsize = s;
}

void TFTLCD::setTextColor(uint16_t c) {             // Œƒ◊÷—’…´
  textcolor = c;
}

size_t TFTLCD::write(uint8_t c) {                       //–¥8Œª ˝æ›
  if (c == '\n') {
    cursor_y += textsize*8;
    cursor_x = 0;
  } else if (c == '\r') {
...
void TFTLCD::drawString(uint16_t x, uint16_t y, char *c, 
  ...
  fillRect(x,y,20*size*6,size*8,0xF800);
  while (c[0] != 0) {
...
void TFTLCD::drawChar(uint16_t x, uint16_t y, char c, 
  ...
  fillRect(x,y,size*6,size*8,0x0000);
